...
func (r *LineReader) Next() (b []byte, n int, err error) {
	...
	for {
		...
		if len(buf) == 0 {
			logger.Error("Empty buffer returned by advance")
			continue
		}
		...
		if bytes.HasSuffix(buf, r.decodedNl) {
			...
			logger.Debugf("Line ending char found which wasn't one: %c", buf[len(buf)-1])
			logger.Debugf("In %s", string(buf))
		}
	}
...
func (r *LineReader) advance() error {
	...
	for idx == -1 {
		...
		if r.maxBytes != 0 {
			...
			for idx != -1 && idx > r.maxBytes {
				logger.Warnf("Exceeded %d max bytes in line limit, skipped %d bytes line", r.maxBytes, idx)
			...
			if idx == -1 && r.inBuffer.Len() > r.maxBytes {
				...
				if err != nil {
					logger.Error("Error skipping until new line, err:", err)
					return err
				}
				logger.Warnf("Exceeded %d max bytes in line limit, skipped %d bytes line", r.maxBytes, skipped)
				r.byteCount += skipped
				idx = r.inBuffer.IndexFrom(r.inOffset, r.nl)
			}
		}
	...
	if err != nil {
		logger.Errorf("Error decoding line: %s", err)
