...
func (r *JSONReader) decode(text []byte) ([]byte, util.MapStr) {
	...
	if err != nil || jsonFields == nil {
		...
		if r.cfg.AddErrorKey {
			jsonFields = util.MapStr{"error": createJSONError(fmt.Sprintf("Error decoding JSON: %v", err))}
		}
...
func (r *JSONReader) Next() (reader.Message, error) {
	...
	message.AddFields(util.MapStr{"json": fields})
	return message, nil
}
...
func createJSONError(message string) util.MapStr {
	return util.MapStr{"message": message, "type": "json"}
}
...
func (p *JSONParser) Next() (reader.Message, error) {
	...
	//} else {
	//	message.AddFields(util.MapStr{p.target: jsonFields})
	//}
