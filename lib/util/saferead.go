...
// Vendored copy of the framework's core/util saferead (framework PR #420,
// instance-log-tail-api). Keep in sync with upstream and switch callers
// back to infini.sh/framework/core/util once the framework release that
// ships it is a hard dependency here.
...
// exist yet, the deepest EXISTING ancestor is resolved instead, so a not-
// yet-created target under a symlinked root canonicalizes identically to
// the root — and a link jumping out of the allowed root cannot smuggle a
// target past the containment check either way. (Vendored from the
// framework's core/util safedelete.go.)
